...
class Solution {
    ...
    private class Graph {
        ...
        Graph(int count, int[][] edges) {
            this.count = count;
            edgeTos = new Set[count];

            for(int[] edge: edges) {
                if (edgeTos[edge[1]] == null) edgeTos[edge[1]] = new HashSet<>();
                edgeTos[edge[1]].add(edge[0]);
            }
        }
        ...
        boolean hasCycle() {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (dfs(i, edgeTos, visited)) return true;
            }
            return false;
        }

        boolean dfs(int i, Set<Integer>[] edges, boolean[] visited) {
            if (edges[i] == null) return false;
            if (visited[i]) return true;
            visited[i] = true;

            for (int adj: edges[i]) {
                if (dfs(adj, edges, visited)) return true;
            }
            visited[i] = false;
